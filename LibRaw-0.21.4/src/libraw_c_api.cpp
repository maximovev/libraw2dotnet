...
#define LIM(x, min, max) MAX(min, MIN(x, max))
  ...
  DllDef int libraw_get_raw_pixel(libraw_data_t *lr, unsigned int index)
  {
        if ((index >= 0) && (index < (lr->sizes.raw_height * lr->sizes.raw_width)))
        {
            //return lr->image[index][color_index];
          return lr->rawdata.raw_image[index];
        }
        else
        {
            return -1;
